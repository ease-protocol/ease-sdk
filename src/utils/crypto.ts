...
export function parseAttestationDocument(attestationDocBase64: string) {
  ...
  const coseSign1Data = decode(rawBytes) as [Uint8Array, {}, Uint8Array, Uint8Array];
  const payload = decode(coseSign1Data[2]) as Record<string, any>;

  const pcrs = Object.entries(Object.fromEntries(payload.pcrs as Map<number, Uint8Array>)).reduce(
    (acc, [k, v]) => {
      acc[k] = bytesToHex(v);
      return acc;
    },
